...
// 	echo '<meta http-equiv="refresh" content="2;url=view_emp.php?tid='.$id.'&&mid='.base64_encode("409").'">';
// 	echo '<br>';
// 	echo'<span class="itext" style="color: #FF0000">Image must not more than 2MB!</span>';
// }
...
$insert = mysqli_query($link, "INSERT INTO borrowers(fname, lname, data_nascimento, sexo, naturalidade, filiacao, estado_civil, residencia, documento, numero_documento, emissao, validade, localEmit, email, phone, addrs1, addrs2, city, state, zip, country, comment, account, balancE, image, date_time, status) VALUES('$fname','$lname', '$data_nascimento', '$sexo', '$naturalidade', '$filiacao', '$estado_civil', '$residencia', '$documento', '$numero_documento', '$emissao', '$validade','$localEmit','$email','$phone','$addrs1','$addrs2','$city','$state','$zip','$country','$comment','$account','0.0','$location',NOW(),'$status')") or die (mysqli_error($link));
if(!$insert)
{
